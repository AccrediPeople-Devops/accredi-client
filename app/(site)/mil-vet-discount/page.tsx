...
import { militaryDiscountService, MilitaryDiscountFormData } from "@/app/components/service/militaryDiscount.service";
...
export default function MilitaryDiscountPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await militaryDiscountService.submitForm(formData as MilitaryDiscountFormData);
    ...
    <div className="min-h-screen site-section-bg">
      ...
      <section className="relative overflow-hidden site-section-bg">
        ...
        <div className="absolute inset-0 overflow-hidden">
          <div className="absolute top-20 left-20 w-96 h-96 bg-[#7C3AED]/5 site-light:bg-[#7C3AED]/10 rounded-full blur-3xl "></div>
            ...
            <div className="site-glass backdrop-blur-xl rounded-3xl p-8 shadow-2xl hover:bg-white/15 site-light:hover:bg-white/70 transition-all duration-500">
              {submitted ? (
                <div className="text-center py-16">
                  ...
                  <p className="site-text-secondary">{submitMessage}</p>
                </div>
              ) : (
                <>
                  {submitMessage && !submitted && (
                    <div className="mb-6 p-4 bg-red-100 border border-red-300 rounded-2xl">
                      <p className="text-red-700 text-center">{submitMessage}</p>
                    </div>
                  )}
