...
class ExamAttemptService {
  ...
  async saveProgress(
    ...
    try {
      const response = await axiosInstance.post(
        `/exam-attempts/save-progress/${attemptId}`,
        {
          answers,
        }
      );
      ...
      throw new Error(
        error.response?.data?.message || "Failed to save exam progress"
      );
    }
  }
  ...
  async submitExam(
    ...
    try {
      const response = await axiosInstance.post(
        `/exam-attempts/submit/${attemptId}`,
        {
          answers,
          endTime,
          timeSpent,
  ...
  async getMyAttempts(): Promise<ExamAttempt[]> {
    try {
      ...
      if (response.data && Array.isArray(response.data)) {
        ...
        response.data &&
        response.data.examAttempts &&
        Array.isArray(response.data.examAttempts)
      ) {
      ...
      throw new Error(
        error.response?.data?.message || "Failed to fetch exam attempts"
      );
    }
  }
  ...
  async getExamResult(attemptId: string): Promise<ExamAttemptResponse> {
    try {
      const response = await axiosInstance.get(
        `/exam-attempts/result/${attemptId}`
      );
      return response.data;
    } catch (error: any) {
  ...
  async resumeExamAttempt(attemptId: string): Promise<ExamAttemptResponse> {
    try {
      const response = await axiosInstance.get(
        `/exam-attempts/resume/${attemptId}`
      );
      return response.data;
    } catch (error: any) {
      console.error("Error resuming exam attempt:", error);
      throw new Error(
        error.response?.data?.message || "Failed to resume exam attempt"
      );
    }
  }
}
