...
import userProfileService, { UserResource, UserResourceContent } from "@/app/components/user-dashboard/services/userProfile.service";
...
export default function ResourcesPage() {
  ...
  const transformResourceToCategory = (userResource: UserResource): ResourceCategory => {
    const resources: Resource[] = userResource.content.map((content: UserResourceContent) => {
      ...
      return {
        ...
        downloadUrl: primaryFile?.path ? `${process.env.NEXT_PUBLIC_API_URL || 'https://api.accredipeoplecertifications.com'}/${primaryFile.path}` : undefined,
      };
    });

    return {
      id: userResource._id,
      title: `Course Resources (${userResource.courseId.slice(-8)})`,
      description: `${resources.length} resource${resources.length === 1 ? '' : 's'} available for download`,
  ...
  const fetchResources = async () => {
    ...
    try {
      ...
      if (response.status && response.resources) {
        ...
        // Filter out deleted resources and transform to categories
        const transformedCategories = response.resources
          .filter(resource => !resource.isDeleted && resource.isActive) // Only active, non-deleted resources
          .filter(resource => resource.content && resource.content.length > 0) // Only resources with content
          .map(transformResourceToCategory);
      ...
      setError(error.response?.data?.message || error.message || "Failed to load resources");
      setCategories([]);
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="bg-[var(--card-bg)] border border-red-200 rounded-[var(--radius-lg)] shadow-sm p-6">
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-medium text-[var(--foreground)] mb-2">Error Loading Resources</h3>
      ...
      {categories.length === 0 ? (
        <div className="bg-[var(--card-bg)] border border-[var(--border)] rounded-[var(--radius-lg)] shadow-sm p-6">
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-medium text-[var(--foreground)] mb-2">No Resources Found</h3>
